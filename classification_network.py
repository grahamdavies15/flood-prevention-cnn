...
if __name__ == "__main__":
    ...
    for filenames, images in dataloader:
        ...
        for i in range(len(filenames)):
            balanced_winter.loc[balanced_winter['file_path'] == filenames[i], 'pred'] = "blocked" if predictions[
                                                                                                         i, 1].item() > threshold else "clear"

        # Save balanced_winter dataframe with predictions
        balanced_winter.to_csv('winter_predictions.csv', index=False)
